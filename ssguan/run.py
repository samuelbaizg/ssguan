...
class ClickContext(object):
    ...
    def __init__(self):
        ...
        self.log_config = os.path.join(os.path.dirname(__file__) , "config", "log.properties")
...
@click.group()
@click.option('--log_config', default=_default_ctx.log_config,
...
def cli(ctx, **kwargs):
    ...
    log.fileConfig(ctx.loggingg_config)
...
def webapi(ctx, host, port):
    ...
    """
    from ssguan.ignitor.web import webapp
    webapp.startup(host, port)
...
def install(ctx):
    ...
    Install ssguan
    """
    from ssguan.wheels import update
    update.install()
